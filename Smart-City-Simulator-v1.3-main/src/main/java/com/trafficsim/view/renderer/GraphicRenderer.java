...
public class GraphicRenderer implements SceneRenderer {
    ...
    public void render(GraphicsContext gc, SimScene scene, double zoom, double panX, double panY) {
        ...
        if (!scene.getIntersections().isEmpty()) {
            minX = scene.getIntersections().stream().mapToDouble(Intersection::getCx).min().orElse(0) - SimConfig.MAP_MARGIN;
            maxX = scene.getIntersections().stream().mapToDouble(Intersection::getCx).max().orElse(0) + SimConfig.MAP_MARGIN;
            minY = scene.getIntersections().stream().mapToDouble(Intersection::getCy).min().orElse(0) - SimConfig.MAP_MARGIN;
            maxY = scene.getIntersections().stream().mapToDouble(Intersection::getCy).max().orElse(0) + SimConfig.MAP_MARGIN;
        }
        ...
        // 3. Draw Roads (Asphalt & Curb in passes to avoid overlap issues)
    ...
    private void drawBackground(GraphicsContext gc, SimScene scene, double minX, double maxX, double minY, double maxY) {
        ...
        for (Intersection i : scene.getIntersections()) {
            if (i.getType() == Intersection.Type.FIVE_WAY) {
                ...
                double rOuter = r + 30;
                gc.setFill(c(CURB));
                gc.fillOval(i.getCx() - rOuter, i.getCy() - rOuter, rOuter * 2, rOuter * 2);
            } else {
    ...
    private void drawCityBlocks(GraphicsContext gc, SimScene scene, double minX, double maxX, double minY, double maxY) {
        ...
        int numCols = (int) ((maxX - minX) / cellSize);
        int numRows = (int) ((maxY - minY) / cellSize);

        double startX = minX + (maxX - minX - numCols * cellSize) / 2.0;
        double startY = minY + (maxY - minY - numRows * cellSize) / 2.0;
    ...
    private void drawIntersection(GraphicsContext gc, Intersection inter) {
        ...
        if (inter.getType() == Intersection.Type.FIVE_WAY) {
            // Asphalt fill (expanded to cover road curbs and form a flared entry)
            double rAsphalt = r + 20;
            gc.setFill(c(ASPHALT));
            gc.fillOval(inter.getCx() - rAsphalt, inter.getCy() - rAsphalt, rAsphalt*2, rAsphalt*2);
